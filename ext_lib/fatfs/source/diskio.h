 ...
 */
uint8_t mmc_daddr_to_pdrv(uint8_t daddr);
 ...
 */
uint8_t mmc_pdrv_to_daddr(uint8_t pdrv);

/**
 * @brief get the lowest available drive number
 * 
 * @param daddr the USB device address of the drive
 * @return uint8_t the next physical drive number, from 0
 */
uint8_t mmc_map_next_pdrv(uint8_t daddr);

/**
 * @brief return the physical drive number in use by the USB device to
 * the availalbe drive number pool
 *
 * @param daddr the USB device address of the drive
 * @return uint8_t the previous physical drive number for daddr or FF_VOLUMES
 * if no drive was mounted to USB address daddr
 */
uint8_t mmc_unmap_pdrv(uint8_t daddr);
